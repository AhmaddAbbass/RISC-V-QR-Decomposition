...
matrixH:    .float 1.0,2.0,3.0,4.0,0.0,1.0,1.5,0.5,1.0,0.0,2.5,2.0,1.0,0.0,2.0,0.0 # 32x32 floats (4*4*4 bytes)
...
main:
    la x10, matrixH      # Load address of matrixA into x10
...
## 4x4
granny:
    ## initially empty cache, men3abbe feha first column of Q matrix hence producing the first q vector , ( q_k).

    ## So we take from main memory whic is the matrix A, put the kth column of A in the cache , then access the cache to put in the vector q_k
    addi sp,sp,-52
    sw x15,0(sp)
    sw x16,4(sp)
    sw x19,8(sp)
...
loop1:
    ...
    li x16, 0  # i = 0
    ## replace each x17 by the address of a_cache
loopcache:

    bge x16, x13, end_loopcache
    slli x19, x16, 2
    add x19, x4, x19  # &a_cache[i]
    slli x20, x16, 2 # 
    add x20, x20, x15
    slli x20, x20, 2
    add x20, x20, x10  # &A[i][k]
    flw f1, 0(x20)
    fsw f1, 0(x19)  # a_cache[i] = A[i][k]
    addi x16, x16, 1
    j loopcache
end_loopcache:
li x16, 0
